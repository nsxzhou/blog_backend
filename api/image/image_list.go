...
func (i *Image) ImageList(c *gin.Context) {
	...
	list, count, err := search_ser.ComList(models.ImageModel{}, search_ser.Option{
		Likes:    []string{"name"},
		PageInfo: req,
	})
	if err != nil {
	...
	global.Log.Info("图片列表成功", zap.String("method", c.Request.Method), zap.String("path", c.Request.URL.Path))
